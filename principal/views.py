...
class _UsuarioArchivadoAdapter:
    ...
    def __init__(self, ua):
        ...
        self.id = ua.id
        self.pk = ua.id
        self.id_original = ua.id_original   # ID del User original
...
class _CursoArchivadoAdapter:
    ...
    def __init__(self, ca, semestre_map=None):
        ...
        self.id = ca.id
        self.pk = ca.id
        self.id_original = ca.id_original   # ID del Curso original
        self.name = ca.name
...
class CursoAcademicoDetailView(DetailView):
    ...
    def _context_semestres_tab(self, curso_academico):
        ...
        for s in archivados:
            ...
            # Contar matrículas del semestre específico (no de todo el curso)
            num_matriculas = MatriculaArchivada.objects.filter(
                semestre_archivado=s
            ).count()
        ...
        if group_name == 'Profesores':
            ...
            if curso_academico_activo:
                    ...
                    if course.status == 'F':
                        ...
                        class _CursoFinalizadoProxy:
                            def __init__(self, curso, sem_actual, sems_arch):
                                ...
                                self.curso_tambien_finalizado = False
                ...
                for course_id_original, matriculas_list in arch_por_curso_original.items():
                    ...
                    curso_obj = Curso.objects.filter(pk=course_id_original).only('id', 'name', 'status').first()
                    if curso_obj is not None:
                        course_display_id = curso_obj.id
                        course_name = curso_obj.name
                        curso_status = curso_obj.status
                    else:
                    ...
                    # Si el curso también está finalizado, marcarlo para mostrarlo en el template
                    proxy.curso_tambien_finalizado = (curso_status == 'F')
...
class MatriculasListView(BaseContextMixin, ListView):
    ...
    def get_queryset(self):
        semestre_id = self.request.GET.get('semestre')
        if semestre_id and str(semestre_id).isdigit():
    ...
    def get_context_data(self, **kwargs):
        ...
        # CA activo para preselección por defecto
        ca_activo = CursoAcademico.objects.filter(activo=True).first()
        context['cursos_academicos'] = CursoAcademico.objects.all().order_by('-activo', '-id')
        context['ca_activo_id'] = ca_activo.id if ca_activo else None

        curso_academico_id = self.request.GET.get('curso_academico')
        # Si no hay filtro explícito de CA, usar el activo como contexto
        ca_id_efectivo = curso_academico_id or (str(ca_activo.id) if ca_activo else None)

        # Filtrar cursos para el selector
        if ca_id_efectivo:
            cursos_en_principal = Curso.objects.filter(
                curso_academico__id=ca_id_efectivo
            ).distinct()
            if cursos_en_principal.exists():
                context['cursos'] = cursos_en_principal
            else:
                ...
                context['cursos'] = CursoArchivado.objects.filter(
                    curso_academico=ca_arch
                ).distinct() if ca_arch else Curso.objects.none()
        else:
            context['cursos'] = Curso.objects.filter(curso_academico__isnull=False).distinct()

        context['estudiantes'] = User.objects.filter(groups__name='Estudiantes')

        # Semestres archivados disponibles para el selector.
        # Solo se muestran semestres del CA seleccionado/activo, nunca de todos los CAs.
        semestres_qs = SemestreCursoArchivado.objects.select_related('curso_archivado').order_by(
            'curso_archivado__name', 'numero_semestre'
        )
        if ca_id_efectivo:
            cursos_ids_principal = list(
            ...
            if cursos_ids_principal:
                semestres_qs = semestres_qs.filter(
                    curso_archivado__id_original__in=cursos_ids_principal
                )
            else:
                ...
                if ca_arch:
                    semestres_qs = semestres_qs.filter(
                        curso_archivado__curso_academico=ca_arch
                    )
                else:
                    semestres_qs = SemestreCursoArchivado.objects.none()
        else:
            semestres_qs = SemestreCursoArchivado.objects.none()
        ...
        _semestre_id_valido = semestre_id and str(semestre_id).isdigit()
...
class CalificacionesListView(BaseContextMixin, ListView):
    ...
    def get_queryset(self):
        ...
        # Sin filtro explícito: delegar toda la lógica a get_context_data
        # (igual que cuando se selecciona un CA explícitamente) para que se
        # combinen las calificaciones vivas con las archivadas del CA activo.
        return Calificaciones.objects.none()

    def get_context_data(self, **kwargs):
        from datos_archivados.models import SemestreCursoArchivado, CalificacionArchivada, CursoArchivado, CursoAcademicoArchivado
        context = super().get_context_data(**kwargs)
        ...
        curso_academico_id = self.request.GET.get('curso_academico')
        # Si no se seleccionó CA explícitamente, usar el CA activo como valor efectivo.
        # Esto garantiza que la carga inicial muestre las calificaciones del CA activo,
        # incluyendo las archivadas por "Terminar Semestre" que ya no están en principal.
        if not curso_academico_id and ca_activo:
            curso_academico_id = str(ca_activo.id)

        # Filtrar cursos para el selector — si el CA está archivado los Curso ya no
        # existen en principal, hay que buscar en CursoArchivado.
        if curso_academico_id:
            cursos_en_principal = Curso.objects.filter(
        ...
        # Semestres archivados disponibles — cuando el CA está archivado los Curso
        # ya no existen, filtrar vía CursoAcademicoArchivado.
        semestres_qs = SemestreCursoArchivado.objects.select_related('curso_archivado').order_by(
            'curso_archivado__curso_academico', 'numero_semestre'
        )
        # Determinar el CA a usar para filtrar semestres: el seleccionado o el activo por defecto.
        # curso_academico_id ya está normalizado al CA activo si no se seleccionó uno.
        if curso_academico_id:
            cursos_ids_principal = list(
                Curso.objects.filter(
                    curso_academico__id=curso_academico_id
...
class StudentCourseAttendanceView(BaseContextMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # El curso puede ya no existir en principal si el CA fue archivado.
        # En ese caso usamos CursoArchivado para obtener el nombre y datos básicos.
        from datos_archivados.models import CursoArchivado as CursoArchivadoModel
        curso_en_principal = Curso.objects.filter(id=course_id).first()
        curso_archivado_ref = None
        if curso_en_principal is None:
            curso_archivado_ref = CursoArchivadoModel.objects.filter(id_original=course_id).order_by('-pk').first()
        course = curso_en_principal or curso_archivado_ref
        ...
        if curso_en_principal is not None:
            semestres_activos = (
                SemestreCursoModel.objects
                .filter(asistencias__student=student, asistencias__course=curso_en_principal)
                .distinct()
                .order_by('numero_semestre')
            )
        else:
            semestres_activos = SemestreCursoModel.objects.none()
...
class CalificacionDetalleEstudianteView(BaseContextMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # El curso puede haber sido eliminado al archivar el CA — buscar en archivados.
        from datos_archivados.models import CursoArchivado as CursoArchivadoModel
        curso_en_principal = Curso.objects.filter(id=course_id).first()

        if curso_en_principal:
            course = curso_en_principal
            from datos_archivados.models import (
                UsuarioArchivado, CalificacionArchivada, CursoArchivado as CursoArchivadoModel2,
            )
            from itertools import chain as _chain
            from decimal import Decimal as _Decimal

            # ── Notas vivas en principal ──────────────────────────────────────
            # Después de "Terminar Semestre" las Calificaciones/Notas se eliminan
            # de principal y se mueven a datos_archivados. Por eso aquí puede no
            # haber nada aunque el Curso siga existiendo.
            calificaciones_qs = Calificaciones.objects.filter(
                student=student, course=course
            ).prefetch_related('notas').order_by('semestre__numero_semestre', 'pk')

            notas_vivas = list(_chain.from_iterable(
                c.notas.all().order_by('fecha_creacion') for c in calificaciones_qs
            ))

            # ── Notas archivadas (semestres terminados del mismo curso) ───────
            # Un CursoArchivado se crea por cada semestre terminado con id_original=curso.pk.
            cursos_archivados = CursoArchivadoModel2.objects.filter(id_original=course_id)
            ua = UsuarioArchivado.objects.filter(usuario_actual=student).first()

            notas_archivadas = []
            avg_archivado = None
            if ua and cursos_archivados.exists():
                cals_arch = CalificacionArchivada.objects.filter(
                    student=ua, course__in=cursos_archivados
                ).prefetch_related('notas_archivadas').order_by(
                    'semestre_archivado__numero_semestre', 'pk'
                )
                notas_archivadas = [
                    _NotaArchivadaAdapter(n)
                    for ca in cals_arch
                    for n in ca.notas_archivadas.all().order_by('fecha_creacion')
                ]
                # Promedio de lo archivado (el último semestre archivado)
                last_arch = cals_arch.last()
                if last_arch:
                    avg_archivado = last_arch.average

            # ── Combinar: archivadas primero (cronológico), luego vivas ───────
            notas = notas_archivadas + notas_vivas

            # Calcular promedio global con todas las notas disponibles
            class _CalificacionMultiProxy:
                def __init__(self, avg):
                    self.average = avg

            if notas:
                valores = [n.valor for n in notas if n.valor is not None]
                global_avg = _Decimal(str(sum(valores) / len(valores))) if valores else None
                calificacion = _CalificacionMultiProxy(global_avg)
            elif calificaciones_qs.exists():
                calificacion = calificaciones_qs.last()
            elif avg_archivado is not None:
                calificacion = _CalificacionMultiProxy(avg_archivado)
            else:
                calificacion = None
            ...
            # Curso archivado: course_id es el id_original del CursoArchivado
            from datos_archivados.models import (
                UsuarioArchivado, CalificacionArchivada, MatriculaArchivada
            )
            course_arch = CursoArchivadoModel.objects.filter(id_original=course_id).order_by('-pk').first()
            if not course_arch:
            ...
            calificacion_arch = CalificacionArchivada.objects.filter(
                student=ua, course=course_arch
            ).prefetch_related('notas_archivadas').first() if ua else None
            ...
            context['student']      = student
            context['course']       = _CursoArchivadoAdapter(course_arch)
            context['calificacion'] = _CalificacionProxy(notas, average) if calificacion_arch else None
            context['notas']        = notas
...
class AsistenciasListView(BaseContextMixin, ListView):
    ...
    def get_queryset(self):
        ...
        # Sin filtro explícito: delegar toda la lógica a get_context_data
        # igual que con calificaciones, para combinar vivas + archivadas.
    ...
    def get_context_data(self, **kwargs):
        from datos_archivados.models import SemestreCursoArchivado, MatriculaArchivada, CursoArchivado, CursoAcademicoArchivado
        context = super().get_context_data(**kwargs)
        ...
        curso_academico_id = self.request.GET.get('curso_academico')
        # Si no se seleccionó CA explícitamente, usar el CA activo como valor efectivo.
        # Así la carga inicial muestra las matrículas/asistencias del CA activo,
        # incluyendo las archivadas por "Terminar Semestre".
        if not curso_academico_id and ca_activo:
            curso_academico_id = str(ca_activo.id)
        ...
        # Semestres disponibles filtrados por el CA efectivo (seleccionado o activo)
        semestres_qs = SemestreCursoArchivado.objects.select_related('curso_archivado').order_by(
            'curso_archivado__curso_academico', 'numero_semestre'
        )
        if curso_academico_id:
...
class AsistenciaDetalleEstudianteView(BaseContextMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # El curso puede haber sido eliminado al archivar el CA.
        from datos_archivados.models import (
            CursoArchivado as CursoArchivadoModel,
            UsuarioArchivado, AsistenciaArchivada,
        )
        curso_en_principal = Curso.objects.filter(id=course_id).first()
        ...
        if curso_en_principal:
            ...
            from datos_archivados.models import (
                UsuarioArchivado, AsistenciaArchivada,
                CursoArchivado as CursoArchivadoModel2,
            )

            # Asistencias vivas en principal (semestre activo actual)
            asistencias_vivas = list(
                Asistencia.objects.filter(
                    student=student, course=course
                ).order_by('date')
            )

            # Asistencias archivadas de semestres anteriores del mismo curso.
            # terminar_semestre() elimina las Asistencias de principal y las mueve
            # a datos_archivados, pero el Curso sigue existiendo. Hay que buscarlas.
            cursos_archivados = CursoArchivadoModel2.objects.filter(id_original=course_id)
            ua = UsuarioArchivado.objects.filter(usuario_actual=student).first()

            asistencias_archivadas = []
            if ua and cursos_archivados.exists():
                arch_qs = AsistenciaArchivada.objects.filter(
                    student=ua, course__in=cursos_archivados
                ).order_by('semestre_archivado__numero_semestre', 'date')
                asistencias_archivadas = [_AsistenciaArchivadaAdapter(a) for a in arch_qs]

            # Combinar: archivadas primero (cronológico), luego vivas
            asistencias = asistencias_archivadas + asistencias_vivas
            ...
            # Curso archivado: course_id es el id_original
            course_arch = CursoArchivadoModel.objects.filter(id_original=course_id).order_by('-pk').first()
            if not course_arch:
                from django.http import Http404
                raise Http404("Curso no encontrado.")
            ua = UsuarioArchivado.objects.filter(usuario_actual=student).first()
            asistencias_arch = AsistenciaArchivada.objects.filter(
                student=ua, course=course_arch
            ).order_by('date') if ua else AsistenciaArchivada.objects.none()
            asistencias = [_AsistenciaArchivadaAdapter(a) for a in asistencias_arch]
            total = len(asistencias)
            presentes = sum(1 for a in asistencias if a.presente)
            ausentes = total - presentes
            porcentaje = round((presentes / total) * 100, 2) if total > 0 else 0
...
class StudentCourseNotesView(BaseContextMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if curso_en_principal is not None:
            semestres_activos = (
                SemestreCursoModel.objects
                .filter(calificaciones__student=student, calificaciones__course=curso_en_principal)
                .distinct()
                .order_by('numero_semestre')
            )
        else:
...
def terminar_semestre_view(request, curso_id):
    ...
    if accion == 'finalizar_curso':
        try:
            # Archivar el semestre activo antes de finalizar el curso, de modo que
            # los datos (matrículas, calificaciones, asistencias) queden guardados
            # en datos_archivados igual que con "Terminar Semestre".
            contadores = terminar_semestre(curso, finalizar=True)
            return JsonResponse({
                'success': True,
                'message': (
                    f"El curso '{curso.name}' ha sido finalizado. "
                    f"Semestre {contadores['semestre_num']} archivado: "
